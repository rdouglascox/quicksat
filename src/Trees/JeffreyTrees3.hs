...
module Trees.JeffreyTrees3  where
...
applyAlphaRules (Node (Just t) ts) = if alphaGo t
    ...
    else let (results,bool) = scan applyAlphaRules ts in
        (Node (Just t) results,bool)

scan :: (PTree -> (PTree,Bool)) -> [PTree] -> ([PTree],Bool)
scan f xs = let ys = map f xs in
    (map fst ys, any snd ys)
...
applyBetaRules (Node (Just t) ts) = if betaGo t
    then (selectRule t $ Node (Just (check t)) ts,True)
    else let (results,bool) = scan applyBetaRules ts in
        (Node (Just t) results,bool)
...
applyDN (Node (Just t) ts) = if dnGo t
    then (selectRule t $ Node (Just (check t)) ts,True)
    else let (results,bool) = scan applyDN ts in
        (Node (Just t) results,bool)
...
contras' :: [Prop] -> Bool
contras' xs = let (pos,neg) = S.partition positive (S.fromList xs) in
   S.intersection pos (S.map negate' neg) /= S.empty

positive :: Prop -> Bool
positive (Negation x) = False
positive _ = True
