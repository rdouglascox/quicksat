...
module Trees.TextBook5 (mktreeSimple, satcheckSimple, prepPTree)  where

-- ordered application of rule, always check for closure (cannonical trees)
...
mktreeSimple2 old t = let new = (applyClosure . applyRuleNB) t in
    if old == new
        then mktreeSimple3 new new
        else mktreeSimple2 new new

mktreeSimple3 :: PTree -> PTree -> PTree
mktreeSimple3 old t = let new = (applyClosure . applyRuleB) t in
    if old == new
        then old
        else mktreeSimple2 new new
